	...
	<el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
		<el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
			...
			<el-form-item prop="userType" label="角色" class="role-list">
				<el-select v-model="dataForm.userType" :placeholder="$t('user.roleIdList')">
					<el-option v-for="role in roleList" :key="role.id" :label="role.name" :value="role.id"></el-option>
				</el-select>
			</el-form-item>
			<el-form-item prop="deptId" label="所属科室" class="role-list">
				<el-select v-model="dataForm.deptId" :placeholder="$t('user.roleIdList')">
			...
			<el-form-item prop="orgId" label="所属机构" class="role-list">
				<el-select v-model="dataForm.orgId" :placeholder="$t('user.roleIdList')">
					<el-option v-for="organ in organList" :key="organ.id" :label="organ.orgName" :value="organ.id"></el-option>
				</el-select>
			</el-form-item>
			<el-form-item prop="cityId" label="所在地区">
				<ren-region-tree v-model="dataForm.pid" placeholder="选择区域" :dept-name.sync="dataForm.parentName"></ren-region-tree>
			</el-form-item>
			<el-form-item prop="address" label="详细地址">
				<el-input v-model="dataForm.address" placeholder="详细地址"></el-input>
			</el-form-item>
			<el-form-item prop="introduce" label="个人介绍">
				<el-input type="textarea" v-model="dataForm.introduce" placeholder="个人介绍"></el-input>
			</el-form-item>
		</el-form>
		...
		dataRule() {
			...
			var validateIdCard = (rule, value, callback) => {
				if (value && !isIdCard(value)) {
					return callback(new Error(this.$t('validate.format', { attr: '证件号格式错误' })))
				}
				callback()
			}
			return {
		...
		dataFormSubmitHandle: debounce(
			function() {
				this.$refs['dataForm'].validate(valid => {
					...
					this.$http
						.post(this.insert ? '/sys/user/add' : '/sys/user/edit', this.dataForm)
						.then(({ data: res }) => {
							if (res.code !== 0) {
								return this.$message.error(res.msg)
							}
